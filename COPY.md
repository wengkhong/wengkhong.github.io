🏠 Home Page Copy
# Weng Khong Lim, PhD

**Associate Professor, Duke-NUS Medical School**  
**Principal Scientist, Genome Institute of Singapore**  
**Director of Bioinformatics, Genomic Medicine Centre (SingHealth Duke-NUS)**

---

## Bridging Genomic Data and Clinical Diagnosis

I develop computational methods and clinical workflows to translate genome sequencing into actionable diagnoses.

My work sits at the intersection of:
- Clinical genomics
- Population-scale sequencing (SG10K, SG100K)
- AI-driven variant interpretation
- Functional validation of uncertain variants

---

## Key Contributions

**Clinical Genomics Implementation**  
Built and deployed bioinformatics pipelines supporting rare disease diagnosis and national genomic programs.

**Population Genomics (SG10K / SG100K)**  
Led analysis of large-scale Asian genome datasets to improve variant interpretation and disease risk prediction.

**AI for Variant Interpretation**  
Developing automated frameworks to prioritise and interpret variants of uncertain significance (VUS) using computational and language models.

**Translational Impact**  
Focus on closing the gap between sequencing and diagnosis, particularly in rare disease and clinical genetics.

---

## Selected Work

- *Nature Communications (2022)* — Analysis of clinically relevant variants in 9,000+ Asian genomes  
- Development of automated ACMG variant interpretation frameworks  
- Clinical bioinformatics pipelines supporting rare disease diagnostics  
- Functional genomics approaches for resolving uncertain variants  

[View Publications →](/publications)  
[View Research →](/research)

---

## Current Focus

- AI-assisted variant prioritisation and interpretation  
- Integration of population genomics into clinical workflows  
- Functionalisation of variants of uncertain significance  
- Scaling genomic medicine in healthcare systems  

---

## Links

- [Google Scholar](#)
- [GitHub](#)
- [Contact](#)
👤 About Page
# About

I am a computational biologist working at the interface of genomics, medicine, and data science.

My research focuses on translating genome sequencing into clinically meaningful insights, particularly in the context of rare disease diagnosis and precision medicine.

---

## Background

I trained in applied mathematics and computational methods before moving into genomics and bioinformatics.

My early work included:
- Semiconductor manufacturing automation (Intel)
- Electronic design automation (EDA)
- Data science in large-scale systems

I later transitioned into genomics, where I now lead bioinformatics efforts across clinical and population-scale sequencing programs.

---

## Current Roles

- Associate Professor, Duke-NUS Medical School  
- Principal Scientist, Genome Institute of Singapore  
- Director of Bioinformatics, Genomic Medicine Centre (SingHealth Duke-NUS)  

---

## Research Philosophy

I view genomics as an engineering problem as much as a biological one.

Sequencing is no longer the bottleneck. Interpretation is.

My work focuses on:
- Building systems that scale
- Reducing ambiguity in variant interpretation
- Integrating computational and experimental approaches
- Applying AI responsibly in clinical contexts

---

## Approach

- Data-driven, but clinically grounded  
- Focus on robustness over novelty  
- Emphasis on real-world deployment  
- Close collaboration with clinicians  

---

## Interests

- Clinical genomics and rare disease  
- Population-scale genetic variation  
- AI and large language models in medicine  
- Functional validation of genetic variants  

---

## Outside Work

I maintain a long-standing interest in science as a way of understanding the world, shaped by a secular and humanistic perspective.

I am also interested in geopolitics, aviation, and model building.
🔬 Research Page
# Research

My work focuses on translating genomic data into clinically actionable insights.

---

## Clinical Genomics

Development of bioinformatics pipelines for rare disease diagnosis and clinical sequencing.

- Variant prioritisation workflows  
- ACMG/AMP-based interpretation frameworks  
- Integration into clinical reporting systems  

---

## Population Genomics

Analysis of large-scale genomic datasets to improve interpretation.

- SG10K and SG100K projects  
- Population-specific variant frequencies  
- Carrier screening and disease risk estimation  

---

## AI and Computational Methods

Development of automated systems for variant interpretation.

- LLM-based literature analysis  
- Automated ACMG criteria evaluation  
- Phenotype-driven gene prioritisation  

---

## Functional Genomics

Resolving uncertain variants through experimental validation.

- Splicing assays (minigene)  
- Cellular models  
- Multi-omics approaches  

---

## Goal

To reduce the gap between sequencing and diagnosis by combining:

- Scalable computation  
- Population data  
- Experimental validation  
- Clinical integration  
📚 Publications Page (Short Version)
# Publications

## Selected Publications

**Lim WK et al. (2022)**  
Analysis of clinically relevant variants from ancestrally diverse Asian genomes  
*Nature Communications*

- 9,000+ whole genomes analysed  
- Identified population-specific pathogenic variants  
- Highlighted gaps in current screening panels  

---

Additional publications available on:

[Google Scholar →](#)
🛠 Tools Page
# Tools and Software

## Variant Interpretation

- Automated ACMG classification frameworks  
- Variant prioritisation pipelines  
- Clinical reporting tools  

## Genomic Analysis

- Population variant annotation workflows  
- Rare disease analysis pipelines  
- Integration with sequencing platforms  

## Ongoing Development

- AI-assisted interpretation systems  
- Literature-driven phenotype extraction  